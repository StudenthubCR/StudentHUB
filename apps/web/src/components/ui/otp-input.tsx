...
// Estilos de foco y estado adaptados al sistema de diseño de StudentHUB
const RING = {
  idle: 'focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/25',
  success: 'border-emerald-500',
  error: 'border-rose-500 ring-2 ring-rose-500/30 delay-150',
} as const
...
const SIZES = {
  sm: {
    box: 'size-9 sm:size-10 rounded-lg',
    text: 'text-base',
    caret: 'h-4.5',
    gap: 'gap-1.5',
    px: 40,
  ...
  md: {
    box: 'size-11 sm:size-12 rounded-xl',
    text: 'text-lg',
    caret: 'h-5.5',
    gap: 'gap-2',
    px: 48,
  ...
  lg: {
    box: 'size-12 sm:size-14 rounded-2xl',
    text: 'text-xl',
    caret: 'h-6.5',
    gap: 'gap-2.5',
    px: 56,
...
const SLOT_CLASS =
  'border border-border bg-surface-alt text-center font-bold text-transparent caret-transparent outline-none transition-all duration-200 selection:bg-transparent disabled:cursor-not-allowed disabled:opacity-50'
...
/**
 * OTP Input adaptado desde Rare UI (swamimalode07/rare-ui) para StudentHUB.
 * Cuenta con animación de deslizamiento de cursor, rodaje de dígitos (rolling characters),
 * vibración en error y trazo SVG animado en confirmación exitosa.
 */
export function OtpInput({
  ...
  return (
    <div
      data-slot="otp-input"
      data-status={status}
      className={cn('relative inline-flex max-w-full justify-center', className)}
      {...props}
    >
      <motion.div
        onFocus={(event) => {
      ...
      >
        {slots.map((slot, index) => (
          ...
          >
            <input
              ...
              disabled={disabled}
              aria-label={`${numeric ? 'Dígito' : 'Carácter'} ${index + 1} de ${length}`}
              className={cn(
                SLOT_CLASS,
                scale.box,
                scale.text,
        ...
        {caretVisible && (
          <motion.span
            ...
            className={cn(
              'pointer-events-none absolute left-0 top-1/2 w-0.5 rounded-full bg-primary',
              scale.caret,
            )}
          />
        )}
